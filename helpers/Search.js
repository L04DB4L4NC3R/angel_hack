...
.then((res)=>{
  ...
  for(var prop of data.response.listings)
      arr.push({name:prop.keywords,contact:"",email:"",location:{lat:prop.latitude,long:prop.longitude},description:prop.summary,address:prop.title,image:prop.img_url});
    resolve(arr);
})
